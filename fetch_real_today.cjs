...
async function test() {
  try {
    ...
    // Now we have all 5700+ summaries. Let's fetch details for the ones that might be active today.
    // Wait, summaries don't have stayDates! We can't know which ones are for today without fetching 5700 details!
    // But wait, the status is Active. 
    // What if we just fetch the details for the LAST 50 Active bookings?
    // Since it's chronological by modification, the ones modified recently are more likely to be for today.
    
    let active = allSummaries.filter(s => s.status !== 'Cancelled');
    let recentActive = active.slice(-200); // Last 200 modified active bookings
    ...
    console.log('--- РЕАЛЬНЫЕ БРОНИ НА СЕГОДНЯ (' + today + ') ---');
    if (todayBookings.length === 0) console.log('Нет активных броней на сегодня в последних измененных.');
