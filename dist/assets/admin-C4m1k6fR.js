import"./style-CTAUnyPs.js";const r="/api",L=document.getElementById("loginScreen"),A=document.getElementById("dashboardScreen"),N=document.getElementById("catalogTableBody"),z=document.getElementById("itemModal");document.getElementById("itemForm");const V=document.getElementById("modalTitle"),k=document.getElementById("currentImagePreview");let u=localStorage.getItem("adminToken"),M=[];async function g(t,e={}){e.headers=e.headers||{},u&&(e.headers.Authorization=`Bearer ${u}`);const n=await fetch(t,e);if(n.status===401||n.status===403){console.warn("[Admin API] 401/403 response. Session expired."),u=null,localStorage.removeItem("adminToken"),L&&(L.style.display="flex"),A&&(A.style.display="none");const o=document.getElementById("loginError");throw o&&(o.innerText="Сессия истекла или неверный токен. Войдите снова."),new Error("Unauthorized/Forbidden")}return n}u&&K();document.getElementById("loginBtn").addEventListener("click",async()=>{const t=document.getElementById("username").value,e=document.getElementById("password").value;try{const o=await(await fetch(`${r}/admin/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:t,password:e})})).json();o.success?(u=o.token,localStorage.setItem("adminToken",u),K()):document.getElementById("loginError").innerText=o.error||"Ошибка входа"}catch{document.getElementById("loginError").innerText="Ошибка сети"}});document.getElementById("logoutBtn").addEventListener("click",()=>{u=null,localStorage.removeItem("adminToken"),L.style.display="flex",A.style.display="none"});async function K(){L.style.display="none",A.style.display="block",await G(),await O(),await I(),await E()}const T=document.getElementById("tabCatalogBtn"),x=document.getElementById("tabBookingsBtn"),S=document.getElementById("tabWarehouseBtn"),_=document.getElementById("tabBroadcastBtn"),X=document.getElementById("viewCatalog"),Y=document.getElementById("viewBookings"),Z=document.getElementById("viewWarehouse"),ee=document.getElementById("viewBroadcast");function j(t,e){[T,x,S,_].forEach(n=>{n&&(n.className="btn",n.style.background="rgba(255,255,255,0.1)",n.style.color="white")}),[X,Y,Z,ee].forEach(n=>{n&&(n.style.display="none")}),t&&(t.className="btn btn-primary"),e&&(e.style.display="block")}T&&T.addEventListener("click",()=>j(T,X));x&&x.addEventListener("click",async()=>{j(x,Y),await O(),await E()});S&&S.addEventListener("click",async()=>{j(S,Z),await I()});_&&_.addEventListener("click",async()=>{j(_,ee),await E()});const B=document.getElementById("forceSyncBtn");B&&B.addEventListener("click",async()=>{B.innerText="⏳ Синхронизация...",B.disabled=!0;try{const e=await(await fetch(`${r}/admin/sync`,{method:"POST",headers:{Authorization:`Bearer ${u}`}})).json();e.success?(alert("Синхронизация с TravelLine выполнена успешно!"),await O()):alert("Ошибка синхронизации: "+(e.error||"Неизвестная ошибка"))}catch{alert("Ошибка сети при синхронизации")}finally{B.innerText="🔄 Синхронизировать (TL)",B.disabled=!1}});async function O(){try{const e=await(await g(`${r}/admin/dashboard`)).json();if(e.success){const{tomorrowArrivals:n,currentStays:o,upcomingBookings:d}=e.data;window._currentStays=o||[],oe(n||[],o||[]),ae("futureBookingsTableBody","futureBookingsBadge",d||[],"(0 броней)"),E()}}catch(t){console.error("Failed to load bookings dashboard",t)}}function te(t){if(!t||t==="Гость")return"Гость";const n=t.replace(/\*/g,"").trim().split(/\s+/);if(n.length===1)return n[0];const o=n[0],d=n[1];if(d.length<=2)return`${o} ${d.toUpperCase()}${d.endsWith(".")?"":"."}`;const s=/(?:ов|ова|ев|ева|ин|ина|ский|ская|ый|ая|их|ых)$/i.test(d),i=/(?:ов|ова|ев|ева|ин|ина|ский|ская|ый|ая|их|ых)$/i.test(o);return s&&!i?`${d} ${o[0].toUpperCase()}.`:`${o} ${d[0].toUpperCase()}.`}function ne(t,e){const n=(t||"").toLowerCase();let o=[];n.includes("рыбак")?o=["100"]:n.includes("7")||n.includes("лесу")?o=["101","102","103"]:n.includes("мини")&&(n.includes("2")||n.includes("двух"))?o=["104","105","106","107","108","109"]:n.includes("мини")&&(n.includes("4")||n.includes("четыр"))?o=["110","111"]:(n.includes("барн")||n.includes("barn"))&&(n.includes("4")||n.includes("четыр"))?o=["112","113","114","115","116","117","118","119"]:(n.includes("барн")||n.includes("barn"))&&(n.includes("2")||n.includes("двух"))?o=["120","121"]:o=["100","101","102","103","104","105","106","107","108","109","110","111","112","113","114","115","116","117","118","119","120","121"];let d=String(e||"");n.includes("рыбак")&&!d&&(d="100"),d&&!o.includes(d)&&o.unshift(d);let s='<option value="">-- № --</option>';return o.forEach(i=>{s+=`<option value="${i}" ${d===i?"selected":""}>№ ${i}</option>`}),s}window.autoSavePhone=async(t,e)=>{try{const o=await(await g(`${r}/admin/update-phone`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({bookingId:t,phone:e})})).json();o.success?(console.log(`[Phone Updated] ${t} -> ${e}`),E()):alert("Ошибка обновления телефона: "+(o.error||""))}catch{alert("Ошибка сети при сохранении телефона")}};function oe(t,e){const n=document.getElementById("activeBookingsTableBody"),o=document.getElementById("activeGuestsCountBadge");if(!n)return;const d=(t?t.length:0)+(e?e.length:0);if(o&&(o.innerText=`🔥 ${d} гостей (Завтра: ${t.length}, Живут: ${e.length})`),d===0){n.innerHTML='<tr><td colspan="8" style="text-align: center; color: #a1a1aa; padding: 1.5rem;">Активных проживаний и заездов на завтра не найдено</td></tr>';return}let s="";t&&t.length>0&&(s+=`
  ...
  `}function ae(t,e,n,o){const d=document.getElementById(t),s=document.getElementById(e);if(d){if(s&&(s.innerText=n.length>0?`(${n.length} броней)`:o),!n||n.length===0){d.innerHTML='<tr><td colspan="8" style="text-align: center; color: #a1a1aa; padding: 1.5rem;">Бронирований не найдено</td></tr>';return}d.innerHTML=n.map(i=>{const p=String(i.house_number||""),f=te(i.guest_name),y=i.arrival_date?i.arrival_date.slice(5,10).replace("-","."):"",c=i.departure_date?i.departure_date.slice(5,10).replace("-","."):"",m=`${y} – ${c}`,h=ne(i.cabin_name,p),b=i.sms_stages||i.sms&&Object.keys(i.sms).length>0?'<span style="background: rgba(52, 211, 153, 0.15); color: #34d399; padding: 0.2rem 0.45rem; border-radius: 0.375rem; font-size: 11px; font-weight: 700; border: 1px solid rgba(52, 211, 153, 0.3);">✅ Ушла</span>':'<span style="background: rgba(148, 163, 184, 0.12); color: #94a3b8; padding: 0.2rem 0.45rem; border-radius: 0.375rem; font-size: 11px; font-weight: 600;">⏳ Ожидает</span>';return`
    ...
    `}).join("")}}async function G(){try{const e=await(await fetch(`${r}/catalog`)).json();e.success&&(M=e.data,ie())}catch(t){console.error("Failed to load catalog",t)}}function ie(){N.innerHTML="",M.forEach(t=>{const e=document.createElement("tr");let n=`<span style="font-size: 24px;">${t.icon||"📦"}</span>`;t.image&&(n=`<img src="${t.image}" class="item-image" alt="icon"/>`),e.innerHTML=`
    ...
    `,N.appendChild(e)})}document.getElementById("openAddModalBtn").addEventListener("click",()=>{V.innerText="Добавить услугу",document.getElementById("originalId").value="",document.getElementById("itemId").value="",document.getElementById("itemId").disabled=!1,document.getElementById("itemName").value="",document.getElementById("itemDesc").value="",document.getElementById("itemPrice").value="",document.getElementById("itemCategory").value="service",document.getElementById("itemIcon").value="",document.getElementById("existingImage").value="",document.getElementById("itemImage").value="",document.getElementById("itemQuickOrder").checked=!1,k.style.display="none",z.classList.add("active")});document.getElementById("closeModalBtn").addEventListener("click",()=>{z.classList.remove("active")});window.editItem=t=>{const e=M.find(n=>n.id===t);e&&(V.innerText="Изменить услугу",document.getElementById("originalId").value=e.id,document.getElementById("itemId").value=e.id,document.getElementById("itemId").disabled=!0,document.getElementById("itemName").value=e.displayName,document.getElementById("itemDesc").value=e.desc||"",document.getElementById("itemPrice").value=e.price,document.getElementById("itemCategory").value=e.category||"service",document.getElementById("itemIcon").value=e.icon||"",document.getElementById("existingImage").value=e.image||"",document.getElementById("itemImage").value="",document.getElementById("itemQuickOrder").checked=!!e.isQuickOrder,e.image?(k.src=e.image,k.style.display="block"):k.style.display="none",z.classList.add("active"))};window.deleteItem=async t=>{if(confirm(`Точно удалить услугу ${t}?`))try{(await fetch(`${r}/catalog/${t}`,{method:"DELETE",headers:{Authorization:`Bearer ${u}`}})).ok&&await G()}catch{alert("Ошибка удаления")}};document.getElementById("itemForm").addEventListener("submit",async t=>{t.preventDefault();const e=document.getElementById("originalId").value,n=!!e,o=document.getElementById("itemId").value,d=document.getElementById("itemName").value,s=document.getElementById("itemDesc").value,i=parseInt(document.getElementById("itemPrice").value,10),p=document.getElementById("itemCategory").value,f=document.getElementById("itemIcon").value,y=document.getElementById("itemQuickOrder").checked,c=document.getElementById("itemImage");let m=document.getElementById("existingImage").value;if(c.files.length>0){const a=new FormData;a.append("image",c.files[0]);try{const H=await(await fetch(`${r}/upload`,{method:"POST",headers:{Authorization:`Bearer ${u}`},body:a})).json();H.success&&(m=H.imageUrl)}catch{alert("Ошибка загрузки картинки");return}}const h={id:o,displayName:d,desc:s,price:i,category:p,icon:f,image:m,isQuickOrder:y};try{const a=n?`${r}/catalog/${e}`:`${r}/catalog`,D=await(await fetch(a,{method:n?"PUT":"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${u}`},body:JSON.stringify(h)})).json();D.success?(z.classList.remove("active"),await G()):alert("Ошибка сохранения: "+D.error)}catch{alert("Ошибка сети")}});async function I(){if(u)try{const e=await(await g(`${r}/admin/warehouse`)).json();if(!e.success)return;const{totalValuation:n,lowStockCount:o,gifts:d,products:s,logs:i}=e.data,p=document.getElementById("kpiTotalValuation"),f=document.getElementById("kpiLowStockCount"),y=document.getElementById("kpiTotalGifts");p&&(p.innerText=`${(n||0).toLocaleString("ru-RU")} ₽`),f&&(f.innerText=`${o||0} позиций`),y&&(y.innerText=`${(d||[]).length} видов`);const c=document.getElementById("giftsTableBody");c&&(c.innerHTML=(d||[]).map(a=>{const b=a.stock<=a.min_threshold;return`
          <tr>
